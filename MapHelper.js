class MapHelper {
    async static makeMapClaim(db, guildID, nationID, mapClaimCode, mapgameBotUtilFunctions) {
        var snapshot
        var status = ""
        await db.ref("discord-servers/" + guildID + "/nations/" + nationID).once("value", (snapshot1) => {
        ...
        await mapgameBotUtilFunctions.generateMapFromMapCode(mapClaimCode, true).then(mapPathAndNumberOfTiles => {
                ...
                if (hoursDifference < 24) {
                    ...
                    if (mapPath == "error parsing map code") {
                        ...
                        mapgameClient.db.ref("discord-servers/" + guildID + "/nations/" + msg.author.id).update({
                            "mapClaimCode": snapshot.val().mapClaimCode + args[0], // TODO: better method of adding new map claims
